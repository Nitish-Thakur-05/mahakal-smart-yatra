...
export function UserProfileDock({ user, onLogout }) {
  ...
  // Hide dock if user is not logged in OR if on Admin page to eliminate duplicate buttons
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ backdropFilter: 'blur(16px)', boxShadow: '0 10px 30px rgba(0,0,0,0.9)' }}
      >
        ...
        >
          <div className="rounded-circle bg-warning text-dark p-1 d-flex align-items-center justify-content-center fw-bold" style={{ width: 26, height: 26, fontSize: '0.78rem' }}>
            {user.name ? user.name[0].toUpperCase() : 'U'}
          </div>
          <span className="text-white fw-bold small" style={{ fontSize: '0.84rem' }}>
            {user.name}
          </span>
        </Link>
